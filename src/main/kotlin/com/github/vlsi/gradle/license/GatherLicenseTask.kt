...
open class GatherLicenseTask @Inject constructor(
    ...
    private fun findPomLicenses(detectedLicenses: MutableMap<ComponentIdentifier, LicenseInfo>) {
        ...
        val licenseNotFound = GradleException("No license found for certain modules")
        val licenses = loadLicenses(componentsWithUnknownLicenses, project)
        for ((id, licenseResult) in componentsWithUnknownLicenses.zip(licenses)) {
            if (licenseResult.isFailure) {
                licenseNotFound.addSuppressed(licenseResult.exceptionOrNull())
                continue
            }
            val licenseTag = licenseResult.getOrNull()!!

            val guessList = nameGuesser.predict(licenseTag.name)
                .entries
                .sortedByDescending { it.value }

            val uri = URI(licenseTag.url)
            ...
            if (matchingLicense != null) {
                logger.debug(
                    ...
                    licenseTag.name, licenseTag.url, matchingLicense.key
                )
        ...
        if (licenseNotFound.suppressed.isNotEmpty()) {
            throw licenseNotFound
        }
    }
